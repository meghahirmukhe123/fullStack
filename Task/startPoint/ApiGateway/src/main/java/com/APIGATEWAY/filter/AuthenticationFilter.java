	...
	public GatewayFilter apply(Config config) {
		...
		return ((exchange, chain) -> {
			if (validator.isSecured.test(exchange.getRequest())) {
				// check it contain header or not.if it contain header then call to token
				// validation
				...
				try {
					...
					// restTemplate.getForObject("http://IDENTITY-SERVICE/validate/"+authHeaders,
					// String.class); //we can use this way also
